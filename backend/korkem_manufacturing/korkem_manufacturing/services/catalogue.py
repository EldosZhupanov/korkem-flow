...
**Позиции заводятся нескладскими, и это граница, а не упущение.** Каталог — это
то, из чего собирается предложение клиенту: кухня, шкаф, прихожая. Такая мебель
делается под заказ и на складе не лежит, поэтому `is_stock_item = 0`, и склад в
заказе у неё пустой — так и должно быть. Материалы (ЛДСП, фурнитура, кромка) —
складские, и они сюда не попадают: их место в спецификации и закупке, этап 6,
где их принесёт импорт из БАЗИС. Заводить материал через этот экран нельзя, и
если однажды понадобится — это отдельное решение, а не флажок в форме.
