...
**Порядок здесь был неверен до 3 сентября 2026, и это выяснил живой сервер.**
Ниже сказано «первое», но первым это быть не может: на чистом пилоте
`chain_smoke` останавливается на второй же строке —

```
frappe.exceptions.ValidationError: В системе нет ни одной группы номенклатуры.
```

потому что ERPNext установлен, но не настроен: ни компании, ни групп
номенклатуры, ни единиц, ни плана счетов. На стенде разработки их заводят
демонстрационные фикстуры, а на пилоте те **отказываются работать** — намеренно,
и это правильно. Значит первым идёт **присвоение узла**
(`services/provisioning.claim`), которое запускает настоящий мастер настройки
ERPNext и создаёт всё перечисленное вместе с компанией и владельцем. Пароль
владельца вводит владелец, в приложении; ни в каком файле его быть не должно.

`/health/ready` до этого честно отвечает `erpnext: down` с причиной
`no_company`. Это не поломка узла, а его пустота — и проверка написана так,
чтобы это было видно.

**Проверено 3 сентября 2026 на api.korkem.asia**, первое настоящее
развёртывание.
...
Проверено на чистой установке 3 сентября: прошёл целиком — **на стенде
разработки**, где мастер-данные ERPNext уже есть. На непри­своенном пилоте он
не проходит вовсе, см. выше.
