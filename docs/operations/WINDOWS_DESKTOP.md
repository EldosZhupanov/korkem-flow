...
`windows/runner/resources/app_icon.ico` **закоммичен** и содержит семь кадров:
16, 24, 32, 48, 64, 128 и 256 пикселей. Кадры настоящие, а не один большой,
потому что панель задач и Alt-Tab рисуют 16–24px, и там кольцо шанырака,
уменьшенное из 256px, превращается в кашу.

`flutter_launcher_icons` для Windows **сознательно не используется** — он
кладёт единственный кадр 256px. В `pubspec.yaml` на этом месте стоит
предупреждение; ключ `windows:` туда возвращать не надо.

Пересобрать из фирменного мастера (Pillow, любой Python 3):

```sh
cd mobile/korkem_flow
python3 -c "
from PIL import Image
Image.open('assets/brand/icon_play.png').convert('RGBA').save(
    'windows/runner/resources/app_icon.ico', format='ICO',
    sizes=[(256,256),(128,128),(64,64),(48,48),(32,32),(24,24),(16,16)])
"
```

Иконка попадает в `.exe` через `windows/runner/Runner.rc`, то есть меняется
только пересборкой. Замена файла у уже собранного `.exe` ничего не даст.
