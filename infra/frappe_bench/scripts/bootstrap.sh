...
# Третий адрес, и его пропуск не виден ничем.
#
# Через `redis_socketio` Frappe публикует всё, что приходит человеку в реальном
# времени: ответ ассистента, обновление экрана, уведомление в интерфейсе.
# `bench init` подставляет сюда `redis://127.0.0.1:13000` — свой собственный
# Redis, которого в этом контейнере нет и не будет.
#
# Ничего при этом не падает. Обработчик отвечает, задание закрывается успешно, в
# журнале «Successfully completed». Событие уходит в мёртвый адрес, socket.io
# его не получает, и человек видит «не удалось связаться с KORKEM» — то есть
# сообщение про сеть там, где сеть ни при чём.
#
# Найдено на первом настоящем узле 4 сентября 2026: владелец написал «привет»
# и получил отказ, а сервер в ту же секунду записал, что модель ответила.
