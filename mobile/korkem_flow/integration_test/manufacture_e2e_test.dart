...
/// Making the goods, then shipping the goods that were made.
///
/// Phase 20 shipped opening stock. This run releases finished units from a
/// work order and then delivers *those*, so the shelf reading before and after
/// is the whole assertion: production must put stock there and delivery must
/// take it away.
///
/// Requires a work order in progress with material in WIP.
///
/// ```sh
/// flutter test integration_test/manufacture_e2e_test.dart -d emulator-5554 \
...
void main() {
  ...
  /// Finished cabinets of the product being made, straight from ERPNext.
      ...
      (await container
              ...
              .getList(
                ...
                const FrappeQuery(
                  ...
                  filters: [
                    FrappeFilter.equals('sales_order', 'SAL-ORD-2026-00003'),
                  ],
                ),
              ))
          .single;
  ...
  testWidgets(
    'a planner releases what was made and ships it',
    (tester) async {
      await signIn(tester);

      final before = await onShelf();
      final planned = await job();
      expect(
        (planned['produced_qty'] as num).toDouble(),
        0.0,
        reason: 'the work order has already produced — reset before this run',
      );

      // 1. Release the finished goods. No quantity named by anyone.
      // The customer, not the product. Two orders make Тумба Караганда, so
      // naming the product is genuinely ambiguous — and the assistant picked
      // the other one the first time this test ran.
      await ask(
        tester,
        'Производство по заказу Павлодар Уют закончено, '
      ...
      await pumpWhile(
        tester,
        () async => await onShelf() == before,
        reason:
            'the finished goods to reach the shelf. '
            'Transcript:\n${transcript()}',
      );

      final made = await job();
      expect((made['produced_qty'] as num).toDouble(), 5.0);
      expect(made['status'], 'Completed');
      expect(
        await onShelf(),
        before + 5.0,
        reason: 'production put nothing on the shelf',
      );

      // 2. And now those very units go out.
      await ask(tester, 'Можно отгрузить этот заказ?');
      await answersWith(tester, '5');

      await ask(tester, 'Отгрузи.');
      await confirmCard(tester, 'sales.create_delivery');

      await pumpWhile(
        tester,
        () async => await onShelf() > before,
        reason: 'the shipment to leave. Transcript:\n${transcript()}',
      );
      expect(
        await onShelf(),
        before,
        reason:
            'what was made is what shipped, so the shelf is back '
            'where it started',
      );
    },
