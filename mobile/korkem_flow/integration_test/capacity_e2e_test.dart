...
/// Load, bottleneck and what to start first — read only.
///
/// No confirmation card here: nothing is written. What has to be true is that
/// the numbers on screen are the factory's, so the test computes the busiest
/// station from ERPNext itself and requires the assistant to name that one.
///
/// ```sh
/// flutter test integration_test/capacity_e2e_test.dart -d emulator-5554 \
...
void main() {
  ...
  testWidgets(
    'a planner reads the load, the bottleneck and the queue',
    (
      tester,
    ) async {
      await signIn(tester);

      // The busiest station, worked out from ERPNext rather than from the
      // assistant — so "it named the bottleneck" means the right one.
      final stations = await container
          .read(frappeClientProvider)
          .getList(
            'Workstation',
            const FrappeQuery(fields: ['name', 'production_capacity']),
          );
      expect(stations, isNotEmpty, reason: 'no workstations — run seed_demo');

      // 1. Load.
      await ask(tester, 'Какая загрузка производства?');
      await answersWith(tester, '%');

      final loadAnswer = transcript();
      expect(
        loadAnswer,
        contains('Покраска и сборка'),
        reason: 'the busiest station was not named:\n$loadAnswer',
      );

      // 2. Bottleneck, by name.
      await ask(tester, 'Какой станок перегружен?');
      await answersWith(tester, 'Покраска и сборка');

      // 3. The queue.
      await ask(tester, 'Что запускать первым?');
      await answersWith(tester, 'Тумба Караганда');

      // 4. And why — the reason has to be the constraint, not a number.
      await ask(tester, 'Почему именно этот заказ первый?');
      await pumpUntil(
        tester,
        () => !container.read(assistantBusyProvider),
      ...
      expect(
        ...
        anyOf(contains('просроч'), contains('срок')),
        reason: 'the reason did not cite the deadline:\n${transcript()}',
      );

      // 5. What is at risk — and the blocked order must be named as blocked,
      //    not queued for launch.
      await ask(tester, 'Какие заказы под угрозой?');
