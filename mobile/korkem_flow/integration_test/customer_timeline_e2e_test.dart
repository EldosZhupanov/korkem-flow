...
/// One customer, end to end — read only.
///
/// The assertions are as much about what must *not* be said. This customer has
/// no CRM deal and nothing shipped, and the planner cannot see the CRM at all,
/// so the run checks the assistant does not invent either.
///
/// ```sh
/// flutter test integration_test/customer_timeline_e2e_test.dart -d emulator-5554 \
...
void main() {
  ...
  testWidgets(
    'a planner reads one customer from deal to delivery',
    (
      tester,
    ) async {
      await signIn(tester);

      // What ERPNext says, so the answers can be checked against it.
      final orders = await container
          ...
          .getList(
            ...
            const FrappeQuery(
              ...
              filters: [FrappeFilter.equals('customer', 'Мебель Астана')],
            ),
          );
      expect(
        orders,
        isNotEmpty,
        reason: 'no orders for the customer — run seed_demo',
      );
      final order = '${orders.first['name']}';
      ...
      final jobs = await container
          ...
          .getList(
            'Work Order',
            FrappeQuery(
              fields: const ['name', 'qty', 'produced_qty'],
              filters: [FrappeFilter.equals('sales_order', order)],
            ),
          );
      expect(jobs, isNotEmpty, reason: 'nothing in production for the order');
      final produced = (jobs.first['produced_qty'] as num).toInt();

      // 1. The whole customer.
      await ask(tester, 'Покажи всё по Мебель Астана.');
      await answersWith(tester, order);
      // The order number streams early and the materials section late, so the
      // whole answer has to land before it can be judged — checking a
      // half-streamed transcript reads as a missing section.
      await pumpUntil(
        tester,
        () => !container.read(assistantBusyProvider),
        reason: 'the whole timeline to arrive',
        timeout: const Duration(minutes: 4),
      );

      final whole = transcript();
      expect(
        whole,
        contains('ДСП 16мм'),
        reason: 'the material problem was not carried through:\n$whole',
      );

      // 2. Production right now.
      await ask(tester, 'Что сейчас происходит с этим заказом?');
      await answersWith(tester, 'Раскрой');

      // 3. Problems — the shortage is real, so it must be named.
      await ask(tester, 'Есть проблемы с этим заказом?');
      await answersWith(tester, 'ДСП 16мм');

      // 4. How much is made — the number has to be the work order's.
      await ask(tester, 'Сколько уже изготовлено?');
      await answersWith(tester, '$produced');
    },
