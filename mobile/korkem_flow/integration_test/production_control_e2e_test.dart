...
/// The whole thing, as somebody who is not the administrator.
///
/// Every earlier phase signed in as `Administrator`, which cannot fail a
/// permission check and so proved nothing about them. This runs the real
/// conversation as `korkem.planner@example.com` — an ordinary System User
/// holding two stock ERPNext roles, `Manufacturing User` and `Stock User`.
///
/// The shape of the conversation is the point. It opens on a question about
/// the floor rather than a document, narrows to the order that is in trouble,
/// asks why, and ends by buying the missing material — and after the first
/// question nothing names an order, an item or a quantity. If the agent cannot
/// carry that, this fails here rather than in a report.
///
/// ```sh
/// flutter test integration_test/production_control_e2e_test.dart -d emulator-5554 \
...
/// ```
///
/// Requires `korkem_manufacturing.seed_demo.seed` (which also creates the
/// user) and no open Material Request against the blocked order — duplicate
/// protection would otherwise refuse the run, correctly and unhelpfully.
void main() {
  ...
  testWidgets(
    'a planner runs the floor, finds the blocked order and buys the material',
    (tester) async {
      await signIn(tester);

      // The user must not be an administrator, or nothing below is evidence.
      ...
      final roles = [
        for (final row in who['message'] as List) '${(row as Map)['role']}',
      ];
      expect(
        roles,
        isNot(contains('System Manager')),
        reason: 'the acceptance run must not be an administrator',
      );
      ...
      expect(
        ...
        0,
        reason:
            'a Material Request already exists — clear it first, or duplicate '
            'protection will (correctly) refuse this run',
      );

      // 1. The floor. No document named.
      await ask(tester, 'Что сейчас происходит в производстве?');
      await answersWith(tester, order);

      // 2. Narrowing. Still no document named.
      await ask(tester, 'Какой заказ заблокирован по материалам?');
      await answersWith(tester, board);

      // 3. Why. The answer has to come from the shortage, not from adjectives.
      await ask(tester, 'Почему?');
      await answersWith(tester, '4');

      // 4. The action, naming neither order, item nor quantity.
      await ask(tester, 'Создай заявку на закупку.');
      ...
      // The call id, taken from the app's own state before the tap.
      //
      // Read here rather than queried from `Pending Action` afterwards: that
      // doctype is System-Manager-only, and widening it so a test could list
      // it would move a real security boundary to suit a test. The client
      // already holds the id it is about to confirm.
      ...
      // Read from state: the transcript is lazy, so a card scrolling out of
      // view empties the finder just as convincingly as confirming does.
      await pumpUntil(
        tester,
      ...
      // Replay: the same proposal, confirmed again, must buy nothing more.
      await expectLater(
        container
            .read(frappeClientProvider)
            .callMethod(
      ...
      expect(
        await requestCount(),
        1,
        reason: 'replay bought the material a second time',
      );
