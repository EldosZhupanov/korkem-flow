...
/// Buying for the whole factory, from a phone.
///
/// Earlier device runs asked about one order. This asks the question a
/// production manager actually opens the app with — what is missing, across
/// everything — and ends by raising a single purchase request for it. Nothing
/// after the first prompt names an order, a material or a quantity.
///
/// ```sh
/// flutter test integration_test/factory_materials_e2e_test.dart -d emulator-5554 \
...
/// ```
///
/// Requires `korkem_manufacturing.seed_demo.seed` and no open Material
/// Request — duplicate protection would otherwise refuse the run, correctly
/// and unhelpfully.
void main() {
  ...
  testWidgets(
    'a planner sees what the factory is missing and buys it in one request',
    (tester) async {
      await signIn(tester);
      expect(
        await requestCount(),
      ...
      // 1. The warehouse question. No order, no material named.
      await ask(tester, 'Что сейчас не хватает на складе?');
      await answersWith(tester, board);

      // 2. A horizon. The server compares the dates, not the model.
      await ask(tester, 'Какие материалы нужно купить на этой неделе?');
      await pumpUntil(
        tester,
        () => !container.read(assistantBusyProvider),
        reason: 'the horizon question to finish',
        timeout: const Duration(minutes: 4),
      );

      // 3. Buy it — still naming nothing.
      await ask(tester, 'Создай заявку на закупку всего необходимого.');
      ...
      // The card must read as a purchase, not as a JSON dump.
      expect(
        find.textContaining(board),
        findsWidgets,
        reason: 'the card must spell out what is being bought',
      );
      expect(
        await requestCount(),
        0,
      ...
      expect(lines, isNotEmpty, reason: 'an empty purchase request was raised');
