...
/// Shipping to the customer — the end of the chain.
///
/// Ten cabinets are owed and six exist, so the interesting assertion is that
/// six leave and four are still outstanding afterwards. The shelf is read from
/// ERPNext before and after; the transcript is not evidence that stock moved.
///
/// ```sh
/// flutter test integration_test/delivery_e2e_test.dart -d emulator-5554 \
...
void main() {
  ...
  Future<List<Map<String, dynamic>>> notes() => container
      .read(frappeClientProvider)
      .getList(
        'Delivery Note',
        const FrappeQuery(
  ...
  testWidgets(
    'a planner ships what is finished and sees what is left',
    (
      tester,
    ) async {
      await signIn(tester);
      expect(
        await notes(),
        isEmpty,
        reason: 'clear delivery notes before this run',
      );
      final before = await onShelf();
      expect(
        before,
        6.0,
        reason: 'expected six finished cabinets — run seed_demo',
      );

      // 1. Can it go out?
      await ask(tester, 'Можно ли сейчас отгрузить заказ Мебель Астана?');
      await answersWith(tester, '6');

      // 2. Ship it. No quantity named by anyone.
      ...
      await pumpWhile(
        ...
        reason: 'the delivery note to be created. Transcript:\n${transcript()}',
      );
      ...
      expect(await onShelf(), before - 6.0, reason: 'the shelf did not empty');

      // 3. What is left — the number has to be ERPNext's.
      await ask(tester, 'Сколько осталось доставить по этому заказу?');
